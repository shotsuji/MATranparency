...
```
# Figure 1 
```{r Figure 1, echo = F, warning=FALSE}
...
ma_sum_all_plot = ggplot(ma_sum_all, aes(y = pc,x = x))+
  ...
  geom_jitter(width = 0.1,height = 0, aes(size=Review_Size,col = factor(x)),alpha = 0.8)+
 ...
 ma_MAType_plot = ggplot(ma_all_predictors, aes(y = pc, x = MA_Type, col = MA_Type))+
   geom_violin(col="grey") +
    geom_boxplot(col = "grey") +
  geom_jitter(width = 0.1,height = 0, aes(size = Review_Size),alpha = 0.7)+
   theme_light(base_size = 15) +
   theme(panel.grid.major.x = element_blank(),
    panel.grid.minor = element_blank()) +
...
#BY YEAR
 ma_sum_byyear_cont_plot = ggplot(ma_byyear_cont_all, aes(y = pc, x = DoP))+
  ...
  stat_smooth(data = ma_all_predictors, method="lm",se=TRUE,col="darkgrey")+
...
png("Fig1.png",width = 600, height = 800)
...
grid.arrange(widths = c(1,1),
  layout_matrix = rbind(c(1,2),
                        c(3,3),
                        c(4,4)),
  ma_sum_all_plot,ma_MAType_plot,ma_sum_byyear_cont_plot,ma_sum_byyear_group_plot)
dev.off()
```

##Figure 2
```{r Figure 2,echo = F, warning=FALSE}
